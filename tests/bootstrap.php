...
spl_autoload_register(function($class) {
    if (0 === strpos($class, 'HealthGraph')) {
        $class = str_replace('HealthGraph', '', $class);
        if ('\\' != DIRECTORY_SEPARATOR) {
            $class = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'HealthGraph' . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $class) . '.php';
        } else {
            $class = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'HealthGraph' . DIRECTORY_SEPARATOR . $class . '.php';
        }
